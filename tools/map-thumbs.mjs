...
for (const id of ['kingsfield', 'darkwood']) {
  ...
  await sharp(src).extract({ left: Math.floor((m.width - side) / 2), top: Math.floor((m.height - side) / 2), width: side, height: side }).resize(256, 256).modulate({ brightness: id === 'darkwood' ? 1.35 : 1.0 }).webp({ quality: 82 }).toFile(`images/maps/${id}.webp`);
