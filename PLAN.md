...
- [x] 19.2 Local co-op - per-device input isolation (WASD / arrows / pad), join slots on the title screen (A to join, Y to change knight, B to leave), centroid camera with zoom from party spread and a soft `PARTY_LEASH` tether at 26, non-blocking per-player level-up picks, downed + bleed-out + proximity revive, HUD row per companion, shared XP pool
